...
|:---  |---  |
| (2) 팀 번호/이름 | 28-e스타트 |
| (3) 구성원 | 유희정(1976250): 리더,*Frontend,...* <br> 이경림(1871033): 팀원,*AI,....* <br> 최윤우(2176387): 팀원,*Backend,....* |
| (4) 지도교수 | 오세은 교수 |
| (5) 트랙  | 산학 |
| (6) 과제 키워드 | 챗봇, RAG, ChatGPT API, 충동 소비(Impulse Buying) CBT(Cognitive Behavioral Therapy) |
| (7) 과제 내용 요약 | 본 프로젝트는 20대 여성의 충동 소비 습관 교정을 목표로 하는 상담형 챗봇 코칭 서비스이다. 기존의 가계부나 예산 관리 애플리케이션은 단순한 지출 기록과 알림 기능에 머물러 있어, 사용자가 소비 습관의 원인을 이해하고 행동을 교정하는 데 한계가 있었다.

이를 해결하기 위해 본 서비스는 ChatGPT API를 활용한 대화형 상담, RAG(Retrieval-Augmented Generation) 기반의 신뢰성 있는 참고 지식 제공, 그리고 CBT(Cognitive Behavioral Therapy) 상담 기법을 결합하였다. 사용자는 앱을 통해 일일 소비 내역을 리포트로 확인하고, 충동 소비 여부를 스스로 체크한 뒤 챗봇 상담을 진행한다. 상담은 “1: 사용자의 상태 파악 → 2: 기법 선택 → 3: 상담 모듈(Covert Sensitization, Cognitive Restructuring, Relapse Prevention) 실행 → 4: 요약”의 구조로 이루어지며, 상담 결과는 자동으로 저장되어 주간 및 월간 리포트로 제공된다.

이를 통해 사용자는 단순히 소비를 기록하는 데 그치지 않고, 자신의 소비 습관을 인지 → 교정 → 유지하는 선순환 과정을 경험할 수 있다. 결과적으로 본 서비스는 개인의 충동 소비 억제뿐 아니라, 장기적으로 지속 가능한 소비 습관 형성에 기여할 것으로 기대된다.  |
